...
example, assume that a user of our class has a variable, my_card, that identifies an instance of the CreditCard class. When the user calls  my_card.get_balance( ), identifier self, within the definition of the get balance method, refers to the card known as my card by the caller. The expression, self. balance refers to an instance variable, named balance, stored as part of that particular credit card’s state.
'''

"""
podemos decir que la palabra clave self permite 
al usuario especificar y acceder a los atributos 
y métodos de una instancia de la clase
"""
...
Internally, this results in a call to the specially named init method that serves as the constructor of the class. Its primary responsibility is to establish the state of a newly created credit card object with appropriate instance variables. In the case of the CreditCard class, each object maintains five instance variables, which we name: customer, bank, account, limit, and balance. 
'''

#Encapsulation
'''
A single leading underscore in the
name of a data member, such as balance, implies that it is intended as nonpublic. Users of a class should not directly access such member
